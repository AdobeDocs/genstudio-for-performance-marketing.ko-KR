...
## 프로젝트별 규칙
- 커서 규칙: 사용자 지정 자동화 및 지침은 `.cursor/rules/*.mdc`에 있습니다. 예: `.cursor/rules/docs-lint.mdc`(lint 프로세스), `.cursor/rules/generate-release-notes.mdc`(`help/user-guide/release-notes.md`을(를) 편집할 때 릴리스 노트 스킬을 가리킴). 자동화된 작업에 대해서는 다음 단계를 따르십시오.
- 에이전트 기술: 작업 워크플로가 `.cursor/skills/<name>/SKILL.md`에 있습니다. 예: `.cursor/skills/generate-release-notes/SKILL.md`(전체 GenStudio 릴리스 노트 프로시저, 같은 폴더의 `examples.md` 및 `reference.md`에 사용).
- 파일 이름 및 주요 내용:
   - 릴리스 정보: `help/user-guide/release-notes.md`에 기존 Experience League 프론트마킹을 유지(`.cursor/skills/generate-release-notes/reference.md#frontmatter` 참조)합니다. 해당 파일을 편집할 때 가상 규칙 `.cursor/rules/generate-release-notes.mdc`이(가) 자동으로 첨부됩니다.
...
- 공개 문서에 Jira ID, 내부 링크 또는 기업 전용 참조를 추가하지 마십시오. `.cursor/skills/generate-release-notes/SKILL.md`(섹션 **금지된 콘텐츠**)을(를) 참조하십시오.
