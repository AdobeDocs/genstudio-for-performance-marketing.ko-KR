...
| **제목** | Meta 광고, 배너 및 디스플레이 광고, LinkedIn | 수신자가 보는 첫 번째 콘텐츠는 관심을 끌어야 합니다. |
...
| **본문** | 이메일, Meta 광고, 배너 및 디스플레이 광고 | 광고의 주요 텍스트가 핵심 메시지를 전달합니다. 관객에게 원하는 행동을 장려하기 위해 매력적이고, 유익하며, 설득력이 있어야 한다. |
| **CTA** | 이메일, Meta 광고, 배너 및 디스플레이 광고, LinkedIn | call-to-action 버튼은 구문과 링크를 사용하여 수신자가 링크를 클릭하거나 구매하는 등의 특정 작업을 수행하도록 장려합니다. |
| **이미지** | 이메일, Meta 광고, 배너 및 디스플레이 광고, LinkedIn | 시각적 호소력을 높이고, 텍스트를 분류하고, 메시지를 지원합니다. 이미지는 고화질로 눈을 사로잡아야 합니다. |
| **바닥글** | 이메일 | 이메일의 아래 섹션에는 연락처 세부 정보, 소셜 미디어 링크, 면책조항 및 구독 취소 옵션과 같은 추가 콘텐츠가 포함되어 있습니다. |
| **텍스트 오버레이** | Meta 광고 | 헤드라인 및 본문 컨텐츠를 지원하고 개선하기 위해 이미지에 배치된 텍스트입니다. |
...
각 [!DNL Content] 보기는 이상적인 자산, 경험 또는 템플릿에 대한 검색 범위를 좁히는 필터 옵션을 제공합니다. 검색 결과의 범위를 좁히기 위해 [지침](/help/user-guide/guidelines/overview.md), [키워드](../content/asset-details.md#user-defined-metadata) 및 [특성 범주](/help/user-guide/insights/attributes.md#categories)를 기반으로 하는 필터가 있습니다.
...
다음은 이메일, 디스플레이 광고, Meta 광고 및 LinkedIn 광고와 같은 채널 유형별 필터를 보여 줍니다.
...
1. _[!DNL Content]_&#x200B;의 상단 표시줄에서&#x200B;**[!UICONTROL HTML 템플릿]**&#x200B;섹션을 선택합니다.

1. **[!UICONTROL + 템플릿 추가]**&#x200B;를 클릭합니다.

1. _[!UICONTROL 승인된 템플릿 추가]_ 창에서 템플릿 유형을 선택합니다. 그런 다음 HTML 템플릿 파일을 찾아보거나 HTML 템플릿 파일을 드롭 공간으로 드래그합니다. **[!UICONTROL 다음]**&#x200B;을 클릭합니다.

   창의 ![템플릿 형식](/help/assets/template-choose-type.png){width="500" zoomable="yes"}

1. _[!UICONTROL 검색된 필드 확인]_ 창에서 필드를 검토합니다. 올바른 템플릿을 사용 중인지 확인하고 모든 세부 정보가 예상대로인지 확인합니다.

   >[!TIP]
   >
   >템플릿의 세부 정보가 올바르지 않으면 **[!UICONTROL 뒤로]**&#x200B;를 클릭하고 이전 단계로 돌아갑니다. 수정된 템플릿 파일을 업로드합니다. 또는 [템플릿 코드 편집기](/help/user-guide/templates/code-editor.md)를 사용하여 간단하게 수정할 수 있습니다.

1. 자동 감지된 필드를 검토합니다. 두 개의 패널 미리 보기/코드 보기에서 왼쪽 패널에 색상이 강조 표시된 검색 영역이 있는 템플릿의 라이브 미리 보기가 표시됩니다.오른쪽 사이드바에는 현재 슬롯 상태와 함께 검색된 모든 필드의 필드 목록이 표시됩니다. 필드 태그가 코드 탭에 표시되는 핸들바 표기법(예: `{{headline}}`, `{{body}}`, `{{image}}`)으로 지정되어 있습니다.

   이메일 템플릿에 대한 미리 보기 예:

   ![미리 보기 필드가 검색됨](/help/assets/template-detected-fields.png){width="650" zoomable="yes"}

1. 영역이 자동으로 검색되지 않으면 미리 보기 패널에서 해당 영역을 마우스로 가리키면 **태그 가리키기** 기능이 표시됩니다. 를 클릭하여 Handlebars 태그를 지정하고 올바른 슬롯 상태를 설정합니다.

   ![로고 필드의 슬롯 상태를 표시하는 마우스로 가리키기 기능](/help/assets/template-slot-state.png){width="500" zoomable="yes"}

   >[!TIP]
   >
   > **슬롯 상태**
   >
   > 감지된 각 필드에 대해 적절한 슬롯 상태를 할당합니다.
   >
   > | **슬롯 상태** | **설명** | **사용 대상** |
   > |---|---|---|
   > | **기타(편집 가능)** | AI는 런타임 시 이 필드에 대한 콘텐츠를 생성하거나 사용자 정의 필드를 수동으로 편집할 수 있습니다. | 헤드라인, 본문 카피, CTA — 어디서든 AI 변형이 필요합니다. 또는 수동 편집 또는 콘텐츠 조각 스왑을 위한 사용자 정의 필드입니다. |
   > | **고정** | 필드는 AI 또는 사용자가 수정할 수 없습니다. | 법적 고지 사항, 규정 각주, 구독 취소 링크. |

1. 또는 [코드] 탭에서 수동으로 HTML을 편집하고 **[!UICONTROL 필드 자동 감지]** 단추를 클릭하여 검색을 다시 실행하고 필드 목록을 업데이트할 수 있습니다.
