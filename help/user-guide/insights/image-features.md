...
| 카메라 각도 | 카메라가 피사체를 포착하는 원근은 뷰어의 이미지 지각과 해석에 영향을 미친다. 이미지 스타일이 `photograph`인 경우 이 특성이 식별됩니다. | `Low angle`, `High angle`, `Eye level`, `Neutral angle`, `Bird's eye view`<p>`eye level`의 예:<p>![눈높이](/help/assets/category/image-camera-angle.png "다른 각도에서 앉아 있는 사람"){width="200" zoomable="yes"} |
