...
이제 Horizon 캔버스는 여러 크기 또는 여러 페이지 템플릿의 크기 및 페이지에 관련 텍스트 및 이미지 필드를 연결하므로 마지막 마일 편집은 한 번만 수행하면 됩니다. 편집 메뉴에서 **[!UICONTROL 적용 대상]** 범위(이 크기, 모든 크기 또는 모든 개념)를 선택하면 편집이 자동으로 전파되며 필요한 경우 필드 태깅을 수동으로 조정할 수 있는 옵션이 제공됩니다.
