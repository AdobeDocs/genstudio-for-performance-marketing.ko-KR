...
텍스트 프롬프트를 사용하여 템플릿에서 사용할 이미지를 생성합니다.

1. **[!UICONTROL 이미지 생성]**&#x200B;을 선택합니다.
1. 드롭다운 메뉴에서 모델을 선택합니다. 생성한 모든 사용자 정의 모델을 선택할 수도 있습니다.
1. 설정 아이콘을 선택하여 생성 설정을 조정합니다.
1. 선택 사항: 종횡비를 선택합니다.
1. 선택 사항: 다음 중 하나를 수행하여 이미지 스타일을 조정합니다.
   * **[!UICONTROL 이미지 업로드]**&#x200B;를 선택하여 장치 또는 AEM에서 참조 이미지를 업로드합니다.
   * **[!UICONTROL 갤러리 찾아보기]**&#x200B;를 선택하여 Adobe 스톡 이미지 중 하나를 선택하십시오.
   * 슬라이더를 사용하여 강도 값을 선택합니다. 강도는 사용자가 제공한 스타일을 Firefly이 얼마나 엄격하게 준수하는지를 조정합니다.
1. **&lt;** 단추를 선택하십시오.
1. 프롬프트를 입력합니다.
1. 생성 아이콘을 선택합니다. 이미지가 플러그인 패널에 나타납니다.
1. 다음 방법 중 하나를 사용하여 이미지를 캔버스로 가져옵니다.
   * 캔버스에 이미지를 끌어서 놓습니다.
   * 그림 캔버스에서 프레임을 선택하고 플러그인 창에서 프레임에 삽입할 이미지를 선택합니다.
   * 업로드 아이콘을 선택하여 캔버스에 이미지를 업로드합니다.
   * 점 3개를 선택하고 **[!UICONTROL 모두 그림으로 다운로드]**&#x200B;합니다.
1. 선택 사항: 추가 작업을 수행하려면 3개의 점을 선택합니다.
   * 프롬프트를 다시 실행하려면 **[!UICONTROL 추가 생성]**&#x200B;을 선택하십시오.
   * **[!UICONTROL 프롬프트 복사]**&#x200B;를 선택하여 프롬프트를 복사합니다.
1. 선택 사항: 연필 아이콘을 선택하여 생성 채우기를 사용하고 단일 이미지에 대해 유사한 작업을 생성합니다.

## 유사 이미지 생성

유사한 이미지 집합을 생성합니다.

1. **[!UICONTROL 유사 항목 생성]** 카드를 선택하십시오.
1. 다음 중 하나를 수행하여 이미지를 참조로 선택합니다.
   * 그림 캔버스에서 이미지를 선택합니다.
   * 장치에서 업로드할 **[!UICONTROL 이미지 업로드]**&#x200B;를 선택하십시오.
   * AEM에서 업로드하려면 **[!UICONTROL AEM 에셋 찾아보기]**&#x200B;를 선택하십시오.
1. 생성 아이콘을 선택합니다. 변형이 플러그인 패널에 나타납니다.
1. 다음 방법 중 하나를 사용하여 이미지를 캔버스로 가져옵니다.
   * 캔버스에 이미지를 끌어서 놓습니다.
   * 그림 캔버스에서 프레임을 선택하고 플러그인 창에서 프레임에 삽입할 이미지를 선택합니다.
   * 업로드 아이콘을 선택하여 캔버스에 이미지를 업로드합니다.
...
1. **[!UICONTROL 배경 제거]** 카드를 선택하십시오.
1. 다음 중 하나를 수행하여 이미지를 참조로 선택합니다.
   * 그림 캔버스에서 이미지를 선택합니다.
   * 장치에서 업로드할 **[!UICONTROL 이미지 업로드]**&#x200B;를 선택하십시오.
   * AEM에서 업로드하려면 **[!UICONTROL AEM 에셋 찾아보기]**&#x200B;를 선택하십시오.
1. **[!UICONTROL 제거]**&#x200B;를 선택합니다. 캔버스에서 이미지를 선택한 경우 이미지가 그림 캔버스에서 바뀝니다. 장치 또는 AEM에서 선택한 이미지인 경우 이미지를 캔버스로 드래그 앤 드롭하거나 **[!UICONTROL 이미지 삽입]**&#x200B;을 선택하여 캔버스에 이미지를 배치할 수 있습니다.

## 생성 채우기

이미지 영역에 생성 채우기를 적용합니다.

1. **[!UICONTROL 생성 채우기]** 카드를 선택하십시오.
1. 다음 중 하나를 수행하여 이미지를 참조로 선택합니다.
   * 그림 캔버스에서 이미지를 선택합니다.
   * 장치에서 업로드할 **[!UICONTROL 이미지 업로드]**&#x200B;를 선택하십시오.
   * AEM에서 업로드하려면 **[!UICONTROL AEM 에셋 찾아보기]**&#x200B;를 선택하십시오.
1. [브러쉬 도구]를 선택하고 마스크를 만듭니다.
1. 선택 사항: 드롭다운 캐럿을 선택하고 브러시 크기를 조정합니다.
1. 재설정 버튼을 선택하여 마스크를 제거합니다.
1. 필요한 경우 배경 제거 아이콘을 선택하여 배경을 제거합니다.
1. 선택한 마스크의 생성을 안내하는 메시지를 입력하고 **[!UICONTROL 생성]** 단추를 선택합니다.
1. 다음 방법 중 하나를 사용하여 이미지를 캔버스로 가져옵니다.
   * 캔버스에 이미지를 끌어서 놓습니다.
   * 그림 캔버스에서 프레임을 선택하고 플러그인 창에서 프레임에 삽입할 이미지를 선택합니다.
   * 업로드 아이콘을 선택하여 캔버스에 이미지를 업로드합니다.
...
텍스트 프롬프트를 사용하여 이미지 콘텐츠를 편집합니다.

1. **[!UICONTROL 편집할 메시지 표시]** 카드를 선택하십시오.
1. 다음 중 하나를 수행하여 이미지를 참조로 선택합니다.
   * 그림 캔버스에서 이미지를 선택합니다.
   * 장치에서 업로드할 **[!UICONTROL 이미지 업로드]**&#x200B;를 선택하십시오.
   * AEM에서 업로드하려면 **[!UICONTROL AEM 에셋 찾아보기]**&#x200B;를 선택하십시오.
1. 설정 아이콘을 선택하여 생성 설정을 조정합니다.
1. 선택 사항: 종횡비를 선택하고 **&lt;** 단추를 선택합니다.
1. 생성을 안내하는 프롬프트를 입력하고 **[!UICONTROL 생성]** 단추를 선택하십시오.
1. 다음 방법 중 하나를 사용하여 이미지를 캔버스로 가져옵니다.
   * 캔버스에 이미지를 끌어서 놓습니다.
   * 그림 캔버스에서 프레임을 선택하고 플러그인 창에서 프레임에 삽입할 이미지를 선택합니다.
   * 업로드 아이콘을 선택하여 캔버스에 이미지를 업로드합니다.
...
이미지의 차원을 확장하고 생성 확장을 사용하여 생성 콘텐츠를 추가합니다. 생성적 확장을 사용하면 맞지 않는 이미지를 배너, Meta 광고, LinkedIn 광고 또는 디스플레이 광고 템플릿에 가장 적합한 종횡비로 변환할 수 있습니다.

1. **[!UICONTROL 생성 확장]** 카드를 선택하십시오.
1. 캔버스에서 이미지를 선택합니다.
1. Gen Expand Temporary 프레임의 크기를 원하는 새 치수로 조정합니다.
1. 선택 사항: 이미지를 프레임 내부의 어느 곳으로든 이동합니다.
1. 생성을 안내하는 프롬프트를 입력하고 **[!UICONTROL 생성]** 단추를 선택하십시오.
1. 캔버스에서 이미지를 선택하여 원본 이미지를 생성된 결과로 바꿉니다.
